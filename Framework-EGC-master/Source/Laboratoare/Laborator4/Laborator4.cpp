...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(0.5f, 0.5f, -3);
	modelMatrix *= Transform3D::Translate(-0.5f, 0.0f, 1.5f);
	modelMatrix *= Transform3D::RotateOY(angularStepOYcube);
	modelMatrix *= Transform3D::Translate(0.5f, 0.0f, -1.5f);
	RenderMesh(meshes["plane50"], shaders["VertexNormal"], modelMatrix);

	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(1.5, 1.5f, -5);
...
void Laborator4::OnInputUpdate(float deltaTime, int mods)
{
	if (window->KeyHold(GLFW_KEY_W)) {
		...
		angularStepOYcube += deltaTime * 5;;
	}
	else if (window->KeyHold(GLFW_KEY_L)) {
		if ((ty > 2) || ok == 1) {
			tx += deltaTime*cos(45);
			ty -= deltaTime*sin(45) - 0.5*9.8*deltaTime*deltaTime;
			ok = 1;
			if (ty <= 0) {
				ok = 0;
			}
